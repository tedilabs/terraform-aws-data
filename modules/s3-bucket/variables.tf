...
variable "objects" {
  description = <<EOF
  (Optional) A configurations of the objects to upload to the S3 bucket. Objects are uploaded from local files or inline contents with the bucket creation. `objects` as defined below.
    (Optional) `files` - A list of files to upload to the S3 bucket. Each file is uploaded as an object from a local file or an inline content. Each object key takes precedence over objects from `objects.directories` when the object keys conflict. Each value of `files` as defined below.
    ...
    (Optional) `directories` - A list of local directories to sync to the S3 bucket. All files matched in each directory are uploaded as individual objects. Objects defined in `objects.files` take precedence over objects from `directories` when the object keys conflict. A key collision between two directories results in a plan-time error. Each value of `directories` as defined below.
    ...
    (Optional) `checksum` - A configurations of the additional checksum for the objects. The checksum is calculated on upload, stored with the objects, and used by S3 to verify the integrity of the object data. `checksum` as defined below.
    ...
    (Optional) `encryption` - A configurations of Server-Side Encryption for the objects. Each attribute only overrides the default encryption configuration of the bucket when explicitly provided. If not provided, the objects follow the default encryption configuration of the bucket. `encryption` as defined below.
    ...
    (Optional) `object_lock` - A configurations of Object Lock for the objects. Object Lock must be enabled on the bucket with versioning to use these settings. To delete the objects locked by a legal hold or a retention via Terraform, `objects.force_destroy` must be enabled. `object_lock` as defined below.
  ...
  type = object({
    ...
    metadata                      = optional(map(string), {})
    provider_default_tags_enabled = optional(bool, false)
  })
  default  = {}
  nullable = false
  ...
  validation {
    condition = alltrue([
      for file in var.objects.files :
      length(file.key) > 0
    ])
    error_message = "`key` of each file cannot be empty."
  }
  validation {
    condition     = length(distinct(var.objects.files[*].key)) == length(var.objects.files)
    error_message = "`key` of each file must be unique within `objects.files`."
  }
  validation {
    condition = alltrue([
      for file in var.objects.files :
  ...
  validation {
    condition = alltrue([
      for directory in var.objects.directories :
  ...
  validation {
    condition     = contains(["CRC32", "CRC32C", "CRC64NVME", "SHA1", "SHA256"], var.objects.checksum.algorithm)
    error_message = "Valid values for `objects.checksum.algorithm` are `CRC32`, `CRC32C`, `CRC64NVME`, `SHA1`, `SHA256`."
  }
  validation {
    condition = (var.objects.encryption.type == null
      ? true
      : contains(["AES256", "AWS_KMS", "AWS_KMS_DSSE"], var.objects.encryption.type)
    )
    error_message = "Valid values for `objects.encryption.type` are `AES256`, `AWS_KMS`, `AWS_KMS_DSSE`."
  }
  validation {
    condition = (var.objects.object_lock.retention == null
      ? true
      : contains(["GOVERNANCE", "COMPLIANCE"], var.objects.object_lock.retention.mode)
    )
    error_message = "Valid values for `objects.object_lock.retention.mode` are `GOVERNANCE`, `COMPLIANCE`."
  }
  validation {
    condition = (var.objects.object_lock.retention == null
      ? true
      : can(formatdate("YYYY", var.objects.object_lock.retention.retain_until_date))
    )
    error_message = "`objects.object_lock.retention.retain_until_date` must be a valid RFC3339 timestamp."
  }
}
