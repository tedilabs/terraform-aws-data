locals {
  ...
  table_bucket_name = split("/", var.table_bucket)[1]
...
###################################################
# Replication for S3 Table
###################################################

# INFO: The replication configuration is a singleton per table. It overrides
# the bucket-level replication configuration for this table.
resource "aws_s3tables_table_replication" "this" {
  count = local.replication_enabled ? 1 : 0

  region = var.region

  table_arn = aws_s3tables_table.this.arn
...
# INFO: Add KMS permissions with `default_service_role.policies` or `default_service_role.inline_policies` if the table is encrypted with KMS.
module "replication_service_role" {
  ...
  name = coalesce(
    ...
    "s3tables-${local.table_bucket_name}-${local.metadata.name}-replication",
  )
      ...
      Statement = [
        {
          ...
          Resource = [var.table_bucket]
        },
        {
          Sid    = "SourceTablePermissions"
          Effect = "Allow"
          Action = [
            "s3tables:GetTable",
          ...
          Resource = [aws_s3tables_table.this.arn]
        },
        {
