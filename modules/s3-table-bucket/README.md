...
| <a name="input_namespaces"></a> [namespaces](#input\_namespaces) | (Optional) A list of names of namespaces to create in the S3 table bucket. Namespaces are used to logically group tables in the table bucket. Each name must be between 1 and 255 characters in length, can consist of lowercase letters, numbers, and underscores, and must begin and end with a lowercase letter or number. | `list(string)` | `[]` | no |
...
| <a name="output_owner_account_id"></a> [owner\_account\_id](#output\_owner\_account\_id) | The account ID of the account that owns the table bucket. |
